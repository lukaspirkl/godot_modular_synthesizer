...
#include "nodes/adsr_node.h"
#include "nodes/metronome_node.h"
#include "../connection_data.h"

void ModularSynthesizerGraphEditor::_bind_methods()
{
	ClassDB::bind_method("_open_context_menu", &ModularSynthesizerGraphEditor::_open_context_menu);
	ClassDB::bind_method("_add_node", &ModularSynthesizerGraphEditor::_add_node);
	ClassDB::bind_method("_connection_request", &ModularSynthesizerGraphEditor::_connection_request);
	ClassDB::bind_method("_disconnection_request", &ModularSynthesizerGraphEditor::_disconnection_request);
	ClassDB::bind_method("_delete_nodes_request", &ModularSynthesizerGraphEditor::_delete_nodes_request);
	ClassDB::bind_method("_scroll_changed", &ModularSynthesizerGraphEditor::_scroll_changed);
}

ModularSynthesizerGraphEditor::ModularSynthesizerGraphEditor() {
	...
	context_menu->add_separator();
	context_menu->add_item("Metronome", NodeData::NodeType::NODE_METRONOME);
	context_menu->add_item("ADSR Envelope", NodeData::NodeType::NODE_ADSR);
	context_menu->add_separator();
...
SynthNode* ModularSynthesizerGraphEditor::_create_node(Ref<NodeData> p_data)
{
	...
	case NodeData::NodeType::NODE_METRONOME:
		return memnew(MetronomeNode(synth, p_data));
	case NodeData::NodeType::NODE_ADSR:
		return memnew(ADSRNode(synth, p_data));
...
void ModularSynthesizerGraphEditor::edit(Ref<ModularSynthesizer> p_synth) {
	if (p_synth.is_valid()) {
		synth = p_synth;
		_refresh_graph();
	}
...
void ModularSynthesizerGraphEditor::_open_context_menu(const Vector2& p_position)
{
...
void ModularSynthesizerGraphEditor::_add_node(int p_id)
{
...
void ModularSynthesizerGraphEditor::_connection_request(const String& p_from, int p_from_index, const String& p_to, int p_to_index)
{
...
void ModularSynthesizerGraphEditor::_disconnection_request(const String& p_from, int p_from_index, const String& p_to, int p_to_index)
{
...
void ModularSynthesizerGraphEditor::_delete_nodes_request()
{
...
void ModularSynthesizerGraphEditor::_scroll_changed(const Vector2& p_scroll)
{
	synth->set_offset(p_scroll / EDSCALE);
}

void ModularSynthesizerGraphEditor::_refresh_graph()
{
